module VsHelper
  ...
  def comparison_result(city_host:city_host, city_away:city_away, item:item)
    ...
    comparison_result = number_to_percentage((city_away[item] - city_host[item])/city_host[item]*100, precision:0)
    symbol = '+' if  city_away[item] > city_host[item]
    return "#{city_away.name_cn} #{symbol}#{comparison_result}"
  end
